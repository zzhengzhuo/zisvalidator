    ...
    fn set<A: ToTokens>(&mut self, error: &Error, obj: A, value: T) {
        ...
        if self.value.is_some() {
            error.push_span_error(
                tokens,
                format!("duplicate validate attribute `{}`", self.name),
            );
        } else {
...
impl Container {
    pub(crate) fn from_ast(error: &Error, item: &syn::DeriveInput) -> Self {
        ...
        for meta_item in item
            .attrs
            .iter()
            .flat_map(|attr| get_validate_meta_items(&error, &attr))
            .flatten()
        {
            match &meta_item {
                ...
                Lit(lit) => {
                    error
                        .push_span_error(lit, "unexpected literal in validate container attribute");
                }
            }
        }
